...
/// Used by Command Mode (Phase 7.1) to capture the user's current selection
/// into the clipboard before the dictated instruction comes in. The caller is
/// responsible for reading the clipboard afterwards (with a small delay so
/// the focused app has time to actually fulfil the copy).
///
/// Layout-independent: uses platform virtual key codes on Win/Mac so it works
/// on AZERTY, DVORAK, Russian, etc., where 'C' might not be where you expect.
/// On Wayland, callers should use the OS-native tool (wtype/dotool/ydotool)
